...
module.exports = class extends Generator {
  ...
  constructor(args, opts) {
    ...
    this.option('folders', {
      type: String,
      default: this.config.get('featureGeneratedFolders') ?? 'services, components, hooks, types, utils, __tests__',
  ...
  writing() {
    ...
    this.folders.split(',').forEach(dir => {
      ...
      const sourceCopy = folder => {
        switch (folder) {
          case 'services':
            return 'index.f.ts.ejs';
          case 'hooks':
            return 'index.hook.ts.ejs';
          case 'types':
            return 'index.type.ts.ejs';
          default:
            return '';
        }
      };
      ...
      if (sourceCopy(folder).length > 1) {
        const indexSubdir = `${folder}/${depascalize(this.name, '-')}.${pluralize.singular(folder)}.ts`;

        this.fs.copyTpl(this.templatePath(sourceCopy(folder)), this.destinationPath(indexSubdir), {
          feature: this.name,
        ...
        this.fs.append(
          ...
          `export * from './${folder}/${depascalize(this.name, '-')}.${pluralize.singular(folder)}';`,
        );
      }
    });

    if (this.input.withComponent) {
      this.log(`generate ${this.options.name} in ${this._generateDestination()}/components`);
      this.composeWith('wow-react:component', {
        ...this.options,
        arguments: [this.options.name],
        path: this.options.path ?? this.destinationPath('components'),
        withComponent: this.input.withComponent,
      });
    }
  }
